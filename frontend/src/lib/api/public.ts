...
import { ApiResponse, ProductDTO, CategoryDTO, PaginatedResponse } from '@/types/product';
...
export async function fetchCategories(): Promise<CategoryDTO[]> {
  try {
    ...
    if (response.data.success) {
      ...
      return response.data.data.filter((cat) => cat.isActive && cat.parentId === null);
    }
...
export async function fetchCategoryTree(): Promise<CategoryDTO[]> {
  try {
    const response = await apiClient.get<ApiResponse<CategoryDTO[]>>(`/public/categories/tree`);
    if (response.data.success) {
      return response.data.data;
    }
 ...
 */
export async function fetchProducts(params: FetchProductsParams): Promise<PaginatedResponse<ProductDTO>> {
  try {
...
export async function fetchProductBySlug(slug: string): Promise<ProductDTO | null> {
  try {
    const response = await apiClient.get<ApiResponse<ProductDTO>>(`/public/products/${slug}`);
    if (response.data.success) {
      return response.data.data;
    }
