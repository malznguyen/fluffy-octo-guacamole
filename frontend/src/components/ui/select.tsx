...
>(({ className, placeholder, ...props }, ref) => {
  ...
  return (
    <span 
      ref={ref} 
      className={cn("flex-1 truncate text-left", !value && "text-slate-400", className)} 
      {...props}
    >
      {label || placeholder}
    </span>
...
>(({ className, children, ...props }, ref) => {
  ...
  const content = (
    <div
      ...
      className={cn(
        "overflow-hidden rounded-md border border-slate-200 bg-white text-slate-950 shadow-lg",
        className
      )}
      {...props}
    >
